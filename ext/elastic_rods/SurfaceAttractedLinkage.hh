#ifndef SURFACEATTRACTEDLINKAGE_HH
#define SURFACEATTRACTEDLINKAGE_HH
...
struct SurfaceAttractedLinkage_T;

using SurfaceAttractedLinkage = SurfaceAttractedLinkage_T<Real>;

template<typename Real_>
struct SurfaceAttractedLinkage_T : public RodLinkage_T<Real_> {
    ...
    SurfaceAttractedLinkage_T() : RodLinkage_T<Real_>() { }

    // Forward all constructor arguments to set(...)
    template<typename... Args>
    SurfaceAttractedLinkage_T(Args&&... args) : RodLinkage_T<Real_>() {
        set(std::forward<Args>(args)...);
    }

    SurfaceAttractedLinkage_T(const SurfaceAttractedLinkage_T &linkage) : RodLinkage_T<Real_>() { set(linkage); } // The above forwarding constructor confuses pybind11
    ...
    void set(const std::string &surface_path, const bool &useCenterline, const std::string &linkage_path,
             size_t subdivision = defaultSubdivision, bool initConsistentAngle = defaultConsistentAngle, InterleavingType rod_interleaving_type = InterleavingType::xshell, std::vector<std::function<Pt3_T<Real_>(Real_, bool)>> edge_callbacks = {}, std::vector<Vec3> input_joint_normals = {}) {
        ...
        if (!Base::hasCrossSectionMesh()) Base::meshCrossSection(); 
    ...
    void set(const std::string &surface_path, const bool useCenterline, std::vector<MeshIO::IOVertex > vertices, // copy edited inside
             ...
             bool initConsistentAngle = defaultConsistentAngle, InterleavingType rod_interleaving_type = InterleavingType::xshell, std::vector<std::function<Pt3_T<Real_>(Real_, bool)>> edge_callbacks = {}, std::vector<Vec3> input_joint_normals = {}) {
    ...
    // Initialize by copying from another SurfaceAttractedLinkage_T
    template<typename Real2_>
    void set(const SurfaceAttractedLinkage_T<Real2_> &linkage) {
        Base::set(linkage);
    ...
    template<typename Real2_>
    void set(const std::string &surface_path, const bool useCenterline, 
             const std::vector<typename RodLinkage_T<Real2_>::Joint> &joints, const std::vector<typename RodLinkage_T<Real2_>::RodSegment> &segments,
    ...
    SurfaceAttractedLinkage_T &operator=(const SurfaceAttractedLinkage_T &b) = delete;
    ...
    void setTargetSurface(const Eigen::MatrixXd &V, const Eigen::MatrixXi &F){ target_surface_fitter.setTargetSurface(*this, V, F); }
    ...
    Real_ energy(SurfaceAttractionEnergyType surface_eType = SurfaceAttractionEnergyType::Full) const {
        ...
        if ((surface_eType == SurfaceAttractionEnergyType::Full) || (surface_eType == SurfaceAttractionEnergyType::Attraction))
            result += (attraction_weight / (m_l0 * m_l0)) * target_surface_fitter.objective(*this);
        return result;
    }

    void add_surface_attraction_gradient(VecX &result) const {
        target_surface_fitter.accumulateGradient(*this, result, attraction_weight / (m_l0 * m_l0));
    }
    ...
    void add_surface_attraction_hessian(CSCMat &H) const {
        ...
        Real weight = (attraction_weight / (m_l0 * m_l0));
    ...
    std::string mangledName() const { return "SurfaceAttractedLinkage<" + autodiffOrNotString<Real_>() + ">"; }
    ...
    virtual ~SurfaceAttractedLinkage_T() { }
protected:
...
#endif /* end of include guard: SURFACEATTRACTEDLINKAGE */
