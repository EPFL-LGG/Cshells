...
*/
////////////////////////////////////////////////////////////////////////////////
#ifndef XSHELLOPTIMIZATION_HH
#define XSHELLOPTIMIZATION_HH
...
struct XShellOptimization : public LinkageOptimization<Object>{
    ...
    XShellOptimization(Object<Real> &flat, Object<Real> &deployed, const NewtonOptimizerOptions &eopts = NewtonOptimizerOptions(), std::unique_ptr<LOMinAngleConstraint<Object>> &&minAngleConstraint = std::unique_ptr<LOMinAngleConstraint<Object>>(), int pinJoint = -1, 
                       bool allowFlatActuation = true, bool optimizeTargetAngle = true, bool fixDeployedVars=true);

    // It is easier to pybind this constructor since there is no need to bind LOMinAngleConstraint.
    XShellOptimization(Object<Real> &flat, Object<Real> &deployed, const NewtonOptimizerOptions &eopts = NewtonOptimizerOptions(), Real minAngleConstraint = 0 , int pinJoint = -1, 
                       bool allowFlatActuation = true, bool optimizeTargetAngle = true, bool fixDeployedVars=true) 
        : XShellOptimization(flat, deployed, eopts, std::make_unique<LOMinAngleConstraint<Object>>(minAngleConstraint), pinJoint, allowFlatActuation, optimizeTargetAngle, fixDeployedVars) {}
    ...
    Real J(const Eigen::Ref<const Eigen::VectorXd> &params, OptEnergyType opt_eType = OptEnergyType::Full) override {
        const size_t nfp = numFullParams();
        std::runtime_error mismatch("Dimension mismatch for the design parameters in objective eval (from xshell optim)");
        if (params.size() != int(nfp))     throw mismatch;
    ...
    void setXShellOptimization(Object<Real> &flat, Object<Real> &deployed) {
...
#include "XShellOptimization.inl"
#endif /* end of include guard: XSHELLOPTIMIZATION_HH */
