...
//  n^j     [     dn^j/d_omega                   0                  0                  0]
//
// To obtain the change in the centerline positions, this Jacobian
// should be composed with the following linear map:
//           pos        e_X     theta^j           n^j
// x_j     [  I    -s_jX 0.5 I     0  crossingNormalOffset * I] [ I 0 ... 0]
// x_{j+1} [  I     s_jX 0.5 I     0  crossingNormalOffset * I] [ jacobian ]
// theta^j [  0          0         I               0          ]
...
struct LinkageTerminalEdgeSensitivity {
    ...
    double s_jX;       // +/- 1
    Real_ normal_sign; // whether the twisting angle is defined by the normal or by the flipped normal
    Real_ crossingNormalOffset; // signed offset magnitude of the edge vector in the normal direction

    static constexpr size_t JacobianRows = 7;
    static constexpr size_t JacobianCols = 6;

    // Derivative of [e^j, theta^j, n^j]^T with respect to [omega, alpha, len_A, len_B]
    Eigen::Matrix<Real_, JacobianRows, JacobianCols> jacobian;

    // hessian[i] holds the Hessian of the i^th component of [e^j, theta^j, n^j]^T
    ...
    void update(const Joint &joint, size_t si, const ElasticRod_T<Real_> &rod, bool updatedSource, bool evalHessian, const Eigen::MatrixBase<Derived> &delta_jparams) {
        ...
        is_A = (s_jB == 0);
        normal_sign = joint.terminalEdgeNormalSign(si);
        ...
        crossingNormalOffset = 0.5 * joint.terminalEdgeNormalOffsetDirection(si) * rod.crossSectionHeight(j);
        ...
        const auto &t  = dc.tangent[j],
                   ...
                   &w  = joint.omega();
        const auto &ns = joint.source_normal(),
                   &n  = joint.normal();
        ...
        const Vec3 dt_dalpha    = n.cross(tX); // (note that n x tX is not necessarily s_jX * d1 since the terminal edge's d2 vector can disagree with joint normal n.)
        ...
        // jacobian.template block<1, 3>(theta_offset, 0) = -d1.transpose() * normal_sign * d_n_d_omega;
        if (variation_around_identity) jacobian.template block<1, 3>(theta_offset, 0) = normal_sign * neg_ns_sinc.cross(d1).transpose();
        else                           jacobian.template block<1, 3>(theta_offset, 0) = normal_sign * (neg_ns_sinc.cross(d1).transpose() - (d1.dot(d_n_d_omega_w_coeff)) * w.transpose() - ((d1.dot(w)) * omcdthsq) * ns.transpose() - ( w_dot_ns * omcdthsq) * d1.transpose());
        ...
        // t.cross(dt_dalpha) = t.cross(s_jX * n.cross(t)) = s_jX * n
        ...
        jacobian.template block<3, 3>(normal_offset, 0) = d_n_d_omega;
        ...
        if (delta_size == 0) {
            ...
            {
                if (variation_around_identity) {
                    const Vec3 half_ns = normal_sign * 0.5 * ns;
                    ...
                    d1_dot_pder2_n_domega_domega_presym  = normal_sign * d1 * (omcdthsq * ns + w_dot_ns * w_tcm2ptsdtp4).transpose()
                              + ((0.5 * normal_sign * (w_dot_ns * eptsmecmftsdtp6 * w_dot_d1 - ttcptsm3sdtp5 * w_cross_ns.dot(d1) - tcmsdtc * ns.dot(d1))) * w + normal_sign * tcmsdtc * ns.cross(d1) + (normal_sign * tcm2ptsdtp4 * w_dot_d1) * ns) * w.transpose();
                    d1_dot_pder2_n_domega_domega_presym.diagonal().array() += (normal_sign * 0.5) * d1.dot(d_n_d_omega_w_coeff);
                }
            }
            ...
            ///////////////////////////////////////////////////////////////////
            // Joint normal hessian
            ///////////////////////////////////////////////////////////////////
            // d^2 n / d_omega d_omega
            {
                std::array<Mat3, 3> d2_n_domega_domega;
                ropt::hess_rotated_vector(w, ns, d2_n_domega_domega);
                for (size_t i = normal_offset; i < normal_offset + 3; ++i) {
                    hessian[i].template block<3, 3>(0, 0) = d2_n_domega_domega[i - normal_offset];
                }
            }
        }
        ...
        // Evaluate directional derivative only
            ...
            delta_jacobian.template block<1, 3>(theta_offset, 0) = (-0.5 * s_jX) * (d_n_d_omega.transpose() * delta_tperp - tperp_dot_dtX_domega.transpose() * tX.dot(d_n_d_omega * delta_omega)) - normal_sign * d1_dot_delta_n_domega
            ...
            // d^2 n / d_omega d_omega
            {
                ...
                ropt::hess_rotated_vector(w, ns, d2_n_domega_domega);
                for (size_t i = 0; i < 3; ++i) {
                    delta_jacobian.template block<1, 3>(normal_offset + i, 0) += d2_n_domega_domega[i] * delta_omega;
                }
            }
        }
    }
