...
struct LOMinAngleConstraint {
    ...
    Real alpha_min(const Object<Real> &l) const {
    ...
    Real eval(const Object<Real> &l) const { return alpha_min(l) - eps; }

    // dc / dx = d/dx alpha_min(x)
    Eigen::VectorXd grad(const Object<Real> &l) const {
        Real exact_min = l.getMinJointAngle();
    ...
    Eigen::VectorXd delta_grad(const Object<Real> &l, const Eigen::VectorXd &delta_x) const {
        Real exact_min = l.getMinJointAngle();
    ...
    bool shouldRelease(const Object<Real> &l, NewtonOptimizer &opt) const {
    ...
    void enforce(Object<Real> &l, NewtonOptimizer &opt, const Real maxAngleStep = 0.02) {
    ...
    bool violated(const Object<Real> &l) const { return eval(l) < 0; }
