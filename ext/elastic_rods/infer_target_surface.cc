...
void infer_target_surface(const RodLinkage &l, Eigen::MatrixXd &V, Eigen::MatrixXi &F, size_t nsubdiv, size_t numExtensionLayers, Eigen::Vector3d scale_factors) {
    ...
    // Manipulate the reconstructed mesh by scaling. 
    // Get centroid.
    Vector3D centroid(0, 0, 0);
    for (size_t vi = 0; vi < vertices.size(); ++vi) {
        centroid += vertices[vi].point;
    }
    centroid /= vertices.size();
    for (size_t vi = 0; vi < vertices.size(); ++vi) {
        vertices[vi].point = (vertices[vi].point - centroid).cwiseProduct(scale_factors) + centroid;
    }
...
void infer_target_surface(const RodLinkage &l, std::vector<MeshIO::IOVertex > &vertices, std::vector<MeshIO::IOElement> &elements, size_t nsubdiv, size_t numExtensionLayers, Eigen::Vector3d scale_factors) {
    Eigen::MatrixXd V;
    Eigen::MatrixXi F;
    infer_target_surface(l, V, F, nsubdiv, numExtensionLayers, scale_factors);
