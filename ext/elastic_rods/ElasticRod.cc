...
template<typename Real_>
VecX_T<Real_> ElasticRod_T<Real_>::energyTwistPerVertex() const {
    const size_t nv = numVertices();
    const auto &dc = deformedConfiguration();
    VecX_T<Real_> result(nv);
    result[0] = result[nv - 1] = 0.0;
    ...
    for (size_t i = 1; i < nv - 1; ++i) {
        Real_ libar2 = m_restLen[i - 1] + m_restLen[i];
        Real_ twistDeviation = dc.theta(i) - dc.theta(i - 1) + dc.referenceTwist[i] - m_restTwist[i];
        result[i] = (m_twistingStiffness[i] / libar2) * twistDeviation * twistDeviation;
    }
...
template<typename Real_>
VecX_T<Real_> ElasticRod_T<Real_>::energyStretchPerEdge() const {
    const size_t ne = numEdges();
    const auto &dc = deformedConfiguration();
    VecX_T<Real_> result(ne);

    for (size_t j = 0; j < ne; ++j) {
        Real_ strainj = dc.len[j] / m_restLen[j] - 1.0;
        result[j] = 0.5 * density(j) * m_stretchingStiffness[j] * strainj * strainj * m_restLen[j];
    }
