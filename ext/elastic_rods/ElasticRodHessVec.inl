...
void ElasticRod_T<Real_>::applyHessEnergy(const VecX &v, VecX &result, bool variableDesignParameters, const HessianComputationMask &mask) const {
    ...
    for (size_t i = 1; i < nv - 1; ++i) {
        ...
        /////////////////////////////////////////////
        if (variableDesignParameters && (mask.designParameter_in || mask.designParameter_out)) {
            /////////////////////////////////////////////
            ...
            if (m_designParameterConfig.restKappa) {
                const size_t rk_offset = theta_offset + ne + ne * m_designParameterConfig.restLen; // index of the first rest kappa variable for the stencil
                const Real_ delta_rkappa = v[rk_offset];
                ...
                for (size_t adj_edge = 0; adj_edge < 2; ++adj_edge) {
                    ...
                    d_2_Eb_d_rest_kappa_d_e.col(0) += scale * d_kappa_k_j_de[0][adj_edge].col(0);
                    d_2_Eb_d_rest_kappa_d_e.col(1) += scale * d_kappa_k_j_de[0][adj_edge].col(1);
                }
                ...
                result.template segment<3>(x_offset    ) += delta_rkappa * (-d_2_Eb_d_rest_kappa_d_e.col(0));
                result.template segment<3>(x_offset + 3) += delta_rkappa * (d_2_Eb_d_rest_kappa_d_e.col(0) - d_2_Eb_d_rest_kappa_d_e.col(1));
                result.template segment<3>(x_offset + 6) += delta_rkappa * (d_2_Eb_d_rest_kappa_d_e.col(1));
                result.template segment<2>(theta_offset) += delta_rkappa * (d_2_Eb_d_rk_d_theta);

                if (mask.designParameter_out) {

                    result[rk_offset] += v.template segment<3>(x_offset    ).dot(-d_2_Eb_d_rest_kappa_d_e.col(0));
                    result[rk_offset] += v.template segment<3>(x_offset + 3).dot(d_2_Eb_d_rest_kappa_d_e.col(0) - d_2_Eb_d_rest_kappa_d_e.col(1));
                    result[rk_offset] += v.template segment<3>(x_offset + 6).dot(d_2_Eb_d_rest_kappa_d_e.col(1));
                    result[rk_offset] += delta_theta.dot(d_2_Eb_d_rk_d_theta);
                    result[rk_offset] += delta_rkappa * d_2_Eb_d_2_rk;

                    if (m_designParameterConfig.restLen) {
                        const size_t rl_offset = theta_offset + ne; // index of the first rest length variable for the stencil
                        const Vec2 delta_rlen = v.template segment<2>(rl_offset);
                        result[rk_offset] += delta_rlen.dot(d_2_Eb_d_rk_d_rl); 
                        result.template segment<2>(rl_offset) += delta_rkappa * (d_2_Eb_d_rk_d_rl); 
