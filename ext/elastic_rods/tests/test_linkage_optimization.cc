...
int main(int argc, const char * argv[]) {
    ...
    XShellOptimization<RodLinkage_T> lopt(l2d, l3d, eopts, std::make_unique<LOMinAngleConstraint<RodLinkage_T>>());
    lopt.target_surface_fitter.setTargetJointPosVsTargetSurfaceTradeoff(l3d, 1.0);
    // XShellOptimization lopt(l2d, l3d, eopts);
    lopt.gamma = 0.5;
    lopt.beta = 10000.0;
    lopt.prediction_order = PredictionOrder::Zero;
...
// TODO: reenable after we determine if this is what is making the
    ...
    // updateRotationParametrizations() (in both XShellOptimization::m_forceEquilibriumUpdate
    ...
    if (lopt.prediction_order == PredictionOrder::Two) {
