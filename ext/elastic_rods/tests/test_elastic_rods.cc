...
void generatePerturbations(const size_t nv, Real pos_epsilon, Real theta_epsilon, Real designParameter_epsilon,
                           std::vector<Vector3D> &centerlinePerturbation,
                           std::vector<Real> &thetaPerturbation,
                           std::vector<Real> &designParameterPerturbation) {
    const size_t ne = nv - 1;
    centerlinePerturbation.resize(nv);
    thetaPerturbation.resize(ne);
    // TODO (Samara 2): Depends on which design parameters are used, this test need to change. Should access the config from here to dynamically control this. 
    // Also need to change when the representation for rest kappa includes the normal curvature and exclude the end vertices.
    designParameterPerturbation.resize(ne + nv);
    ...
    for (size_t j = 0; j < ne; ++j) {
        ...
        designParameterPerturbation[j] = designParameter_epsilon * randUniform();
    }
}
...
Eigen::VectorXd getDoFPerturbation(const std::vector<Vector3D> &centerlinePerturbation,
                                   ...
                                   const std::vector<Real> &designParameterPerturbation = std::vector<Real>()) {
    Eigen::VectorXd dofPerturbation(3 * centerlinePerturbation.size() + thetaPerturbation.size() + designParameterPerturbation.size());
    size_t offset = 0;
    ...
    for (Real val : designParameterPerturbation) dofPerturbation[offset++] = val;

    return dofPerturbation;
}

void fdHessianTest(ElasticRod &e, Real pos_epsilon, Real theta_epsilon, Real designParameter_epsilon = 0) {
    const size_t nv = e.numVertices(), ne = e.numEdges();

    std::vector<Vector3D> centerlinePerturbation;
    std::vector<Real>     thetaPerturbation, designParameterPerturbation;
    generatePerturbations(nv, pos_epsilon, theta_epsilon, designParameter_epsilon, centerlinePerturbation, thetaPerturbation, designParameterPerturbation);
    auto dofPerturbation = getDoFPerturbation(centerlinePerturbation, thetaPerturbation, designParameterPerturbation);
    ...
    for (size_t j = 0; j < ne; ++j)  rlens[j] +=    designParameterPerturbation[j];
    ...
    for (size_t j = 0; j < ne; ++j)  rlens[j] -= 2 *    designParameterPerturbation[j];
...
void fdGradientTest(ElasticRod &e, Real pos_epsilon, Real theta_epsilon, Real designParameter_epsilon = 0) {
    const size_t nv = e.numVertices(), ne = e.numEdges();
    std::vector<Vector3D> centerlinePerturbation;
    std::vector<Real>     thetaPerturbation, designParameterPerturbation;
    generatePerturbations(nv, pos_epsilon, theta_epsilon, designParameter_epsilon, centerlinePerturbation, thetaPerturbation, designParameterPerturbation);
    ...
    for (size_t j = 0; j < ne; ++j) {
        analytic_diff_energies += Vector3D(gradStretch.gradTheta  (j) *   thetaPerturbation[j],
                               ...
                               +  Vector3D(gradStretch.gradDesignParameters(j) * designParameterPerturbation[j],
                                           gradBend   .gradDesignParameters(j) * designParameterPerturbation[j],
                                           gradTwist  .gradDesignParameters(j) * designParameterPerturbation[j]);
    }
    ...
    for (size_t j = 0; j < ne; ++j) {
        ...
        restLens[j] += designParameterPerturbation[j];
    }
    e.setDeformedConfiguration(points, thetas);
    ...
    for (size_t j = 0; j < ne; ++j) {
        ...
        restLens[j] -= 2 * designParameterPerturbation[j];
    }
    e.setDeformedConfiguration(points, thetas);
    e.setRestLengths(restLens);
...
void fdMassTest(ElasticRod &r, Real pos_epsilon, Real theta_epsilon) {
    ...
    std::vector<Vector3D> centerlinePerturbation;
    std::vector<Real>     thetaPerturbation, designParameterPerturbation;
    generatePerturbations(nv, pos_epsilon, theta_epsilon, 0, centerlinePerturbation, thetaPerturbation, designParameterPerturbation);
...
int main(int argc, const char * argv[]) {
    ...
    std::cout << std::endl;
    {
        std::vector<Vector3D> centerlinePerturbation;
        std::vector<Real>     thetaPerturbation, designParameterPerturbation;
        generatePerturbations(e.numVertices(), 1e-5, 1e-5, 1e-5, centerlinePerturbation, thetaPerturbation, designParameterPerturbation);
        auto dofPerturbation = getDoFPerturbation(centerlinePerturbation, thetaPerturbation, designParameterPerturbation);
